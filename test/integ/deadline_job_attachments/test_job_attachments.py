...
def _make_long_dir_under(root: Path, target_length: int) -> Path:
    r"""
    Builds a real directory under `root` whose path is at least `target_length` characters.

    The directories are created through the \\?\ prefix, because creating them is itself a
    filesystem operation subject to MAX_PATH.
    """
    long_dir = root
    while len(str(long_dir)) < target_length:
        long_dir = long_dir / "aaaaaaaaaa"
    os.makedirs(WINDOWS_UNC_PATH_STRING_PREFIX + str(long_dir), exist_ok=True)
    return long_dir


def _write_through_prefix(path: Path, contents: bytes) -> None:
    """Writes a file at a path that may exceed MAX_PATH."""
    with open(WINDOWS_UNC_PATH_STRING_PREFIX + str(path), "wb") as f:
        f.write(contents)
...
@pytest.mark.skipif(
    sys.platform != "win32",
    reason="The long path prefix is Windows-only.",
)
...
def test_long_path_is_accepted_by_the_filesystem(tmp_path: WindowsPath, separator: str):
    r"""
    A path over MAX_PATH must open after `_get_long_path_compatible_path` rewrites it.

    This is the assertion the unit tests cannot make. They patch `sys.platform`, so they
    pin the prefix *string construction* and never hand the result to Windows -- a test
    that only compares strings would assert a malformed prefix just as happily as a
    correct one. Here the rewritten string goes to a real `os.open`, so a form Windows
    ...
    slashes is passed to the filesystem verbatim and fails. Callers do supply them --
    `os.path.join` output on manifest-derived relative paths is one source.

    Note this test does not distinguish the fix from a host that merely has
    LongPathsEnabled set, because `python.exe` is manifest-aware and so honours that
    setting. It pins that the string we build is one Windows accepts, which is the part
    unit tests cannot reach.
    """
    long_dir = _make_long_dir_under(tmp_path, WINDOWS_MAX_PATH_LENGTH)
    long_input = long_dir / "scene.ma"

    contents = b"long path input file"
    _write_through_prefix(long_input, contents)

    requested_path = str(long_input).replace("\\", separator)
    assert len(requested_path) + TEMP_DOWNLOAD_ADDED_CHARS_LENGTH > WINDOWS_MAX_PATH_LENGTH, (
        f"Test setup must exceed the {WINDOWS_MAX_PATH_LENGTH} char limit to be meaningful, "
        f"got {len(requested_path)}"
    )

    try:
        # WHEN
        compatible_path = _get_long_path_compatible_path(requested_path)

        # THEN
        # os.open rather than the higher-level helpers in upload.py: those swallow OSError
        # into a None yield, which would report a rejected prefix as an unexplained None.
        fd = os.open(str(compatible_path), os.O_RDONLY)
        try:
            assert os.read(fd, len(contents)) == contents
        finally:
            os.close(fd)
    finally:
        shutil.rmtree(WINDOWS_UNC_PATH_STRING_PREFIX + str(tmp_path), ignore_errors=True)
...
@pytest.mark.skipif(
    is_windows_non_admin(),
    reason="Reaching the built-in C$ administrative share requires Administrator.",
)
def test_long_network_path_is_accepted_by_the_filesystem(tmp_path: WindowsPath):
    r"""
    A long path on a network share must open, which requires the \\?\UNC\ form.

    Prepending \\?\ verbatim to \\server\share yields \\?\\\server\share, which Windows
    rejects outright -- so before the fix a long path on shared storage did not merely
    stay long, it became malformed. Studio asset roots are commonly UNC rather than a
    mapped drive, so this is the shape that matters for shared storage.

    Unlike the drive-letter case, this one does isolate the fix: LongPathsEnabled cannot
    rescue a malformed prefix, so the test fails on any host without the \\?\UNC\
    conversion.

    Uses the built-in C$ administrative share to obtain a genuine UNC path without
    creating one, which keeps the test free of `net share` setup and teardown.
    """
    long_dir = _make_long_dir_under(tmp_path, WINDOWS_MAX_PATH_LENGTH)
    long_input = long_dir / "scene.ma"
    ...
    drive, drive_relative = os.path.splitdrive(str(long_input))
    share_root = f"\\\\localhost\\{drive[0]}$"
    unc_path = share_root + drive_relative
    assert len(unc_path) + TEMP_DOWNLOAD_ADDED_CHARS_LENGTH > WINDOWS_MAX_PATH_LENGTH

    try:
        # Skip rather than fail if the share is unreachable, e.g. the Server service is
        # disabled. That is an environment property and says nothing about the prefix, and
        # this test runs on a schedule where a failure for that reason would be noise.
        try:
            os.stat(share_root + "\\")
        except OSError as e:
        ...
        # WHEN
        compatible_path = _get_long_path_compatible_path(unc_path)

        # THEN
        assert str(compatible_path).startswith(WINDOWS_UNC_DEVICE_PATH_STRING_PREFIX), (
            f"A network path must take the \\\\?\\UNC\\ form, got {compatible_path}"
        )
        fd = os.open(str(compatible_path), os.O_RDONLY)
        try:
            assert os.read(fd, len(contents)) == contents
